<template>
  <v-container>
          ...
          <v-stepper-content step="1">
            <v-list color="transparent" class="font-roboto-condensed" shaped>
              <v-list-item-group v-model="report_type">
                <v-list-item v-for="(report, r) in report_types" :key="`${report.type}-${r}`" :value="report.type" active-class="primary--text text--accent-4" @click="step = 2">
                  <v-list-item-avatar>
              ...
              <v-col cols="12" sm="6">
                ...
                >
                  ...
                  >
                    ...
                    <v-btn text @click="$refs.start_dialog.save(start)" v-text="$t('ok')" />
                  </v-date-picker>
                </v-menu>
              </v-col>
              <v-col cols="12" sm="6">
                ...
                >
                  ...
                  >
                    ...
                    <v-btn text @click="$refs.end_dialog.save(end)" v-text="$t('ok')" />
                  </v-date-picker>
                </v-menu>
              </v-col>
            </v-row>
            ...
            <v-card flat>
              ...
              <v-toolbar color="transparent" flat>
                ...
                <v-dialog v-model="save_dialog" max-width="650" scrollable>
                  <template #activator="{ on }">
                    <v-btn :title="$t('save_configuration')" large icon v-on="on">
    ...
    <v-row no-gutters>
      ...
      <v-btn :disabled="step === 4" color="primary" @click="step++">
        {{ $t('next_step') }}
      </v-btn>
    </v-row>
  </v-container>
...
export default {
  ...
  watch: {
    profile () {
      ...
      this.auto_send = this.profile.auto_send
      this.centers = this.profile.centers
      this.columns = this.profile.columns
      this.email_recipients = this.profile.email_recipients
      this.report_schedule = this.profile.report_schedule
      this.report_title = this.profile.report_title
      this.report_type = this.profile.report_type
      this.start = this.profile.start
      this.end = this.profile.end
    },
    async report_type () {
