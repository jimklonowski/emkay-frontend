<template>
  <v-card :loading="loading" outlined>
    ...
    <v-tabs
      ...
      show-arrows
    >
      <v-tab v-for="(category, c) in categories" :key="`tab-${c}`">
        {{ category.text }}
    ...
    <v-tabs-items v-model="tab">
      <v-tab-item v-for="(category, c) in categories" :key="`tabitem-${c}`">
        <v-simple-table dense>
          <template #default>
            <thead>
              <tr>
                <th v-for="(header, h) in headers" :key="`${header.text}-${h}`" :style="{ minWidth: header.width + 'px' }" v-text="header.text" />
              </tr>
            </thead>
            <tbody>
...
<script>
/**
 * Manage eNotifications page will allow user to setup/toggle/disable various enotifications across different categories
 */
export default {
  ...
  computed: {
    ...
    /**
     * Translated list of days of week
     */
    days_of_week () {
  ...
  methods: {
    save () {
      // TODO: collect this categories inputs and send to server
      const cat = this.categories[this.tab].text
      this.$snotify.info(`${cat} eNotifications...`, 'todo: saving')
    }
  },
  head () {
